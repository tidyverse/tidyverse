...
Data __transformation__ is supported by the core [dplyr](https://dplyr.tidyverse.org/) [@R-dplyr] package. dplyr provides verbs that work with whole data frames, such as `dplyr::mutate()` to create new variables, `dplyr::filter()` to find observations matching given criteria, and `dplyr::left_join()` and friends to combine multiple tables. dplyr is paired with packages that provide tools for specific column types: 
